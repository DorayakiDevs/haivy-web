...
export default function SettingsPage() {
  ...
  function notImplemented() {
    toaster.note("This function is not ready! Sorry for your inconvenience!");
  }

  const { client, auth } = useServices();
  const { user, userDetails: account } = auth;
  ...
  async function saveProfileImage() {
    ...
    try {
      toaster.note("Updating your profile image!", 1e6);
      const res = await updateImageProfile(token, imageFile);

      console.log(res.data);

      toaster.success(
        "Profile image uploaded! This may some time to take effect"
      );
      setImageFile(null);
    } catch (e) {
      toaster.error("An error occured while updating your profile!");
  ...
  async function saveChanges() {
    try {
      ...
      const { error } = await client.rpc("edit_user_details", {
      ...
      toaster.success("Your profile has been saved");
  ...
  return (
    <div className="content-wrapper p-8 pl-0">
      <input
        className="hidden"
        type="file"
          ...
          <div className="flex gap-8 pl-2">
            <div className="w-59 tactr">
              <div className="relative rounded-xl overflow-hidden">
                <div
                  style={{ backgroundImage: getUserAvatar(account) }}
            ...
            <div className="infomation flex gap-8 flex-1">
              <div className="flex coll gap-2 w-full">
                ...
                <DatePicker
                  ...
                  onChange={(d) => bday.setValue(format(d, "yyyy-MM-dd"))}
                  dateOnly
                />
              </div>
            </div>
